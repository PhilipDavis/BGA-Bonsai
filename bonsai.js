    ...
    return declare(`bgagame.${BgaGameId}`, ebg.core.gamegui, {
        ...
        {            
            ...
            this.toolTipText = mapValues({
                'bon_goal-set-1-title': _('Bujin-ji Style Goal'),
                'bon_goal-set-2-title': _('Moyogi Style Goal'),
                'bon_goal-set-3-title': _('Chokkan Style Goal'),
                'bon_goal-set-4-title': _('Shakan Style Goal'),
                'bon_goal-set-5-title': _('Kengai Style Goal'),
                'bon_goal-set-1': _('Your bonsai must have *${n}* wood tiles, including the starting bud tile.'),
                'bon_goal-set-2': _('Your bonsai must have *${n}* leaf tiles, adjacent to one another.'),
                'bon_goal-set-3': _('Your bonsai must have *${n}* fruit tiles.'),
                'bon_goal-set-4': _('Your bonsai must have *${n}* flower tiles that protrude from the *same side* of the Pot (it does not matter if it\'s the right or the left side).'),
                'bon_goal-13': _('Your bonsai must have a tile which *protrudes* out of the pot (on the side with the gold crack).'),
                'bon_goal-14': _('Your bonsai must have tiles *protruding* from *both sides* of the pot.'),
                'bon_goal-15': _('Your bonsai must have a tile *protruding* out of the pot on *one side*, and another tile *below* the pot on the *other side* (the specific sides do not matter).'),
                'bon_goal-warning': _('You may only claim *one Goal tile per color*'),
                'bon_goal-status-1': _('You are *not yet eligible* for this goal (*${n}* more)'),
                'bon_goal-status-2': _('You may *claim* this goal'),
                'bon_goal-status-3': _('You have *renounced* this goal'),
                'bon_goal-status-4': _('You have *claimed* this goal'),
                'bon_goal-status-5': _('You have already claimed a goal of this type'),
                'bon_goal-status-6': _('Your opponent claimed this goal'),
                'bon_goal-set-1-short': _('You need *${n}* more wood tiles to qualify'),
                'bon_goal-set-2-short': _('You need *${n}* more adjacent leaf tiles to qualify'),
                'bon_goal-set-3-short': _('You need *${n}* more fruit tiles to qualify'),
                'bon_goal-set-4-short': _('You need *${n}* more flowers on the same side to qualify'),
                'bon_goal-set-5-short': _('You need *${n}* more criteria to qualify'),
                'bon_slot-0-info': _('If you draw the card in this position, you do not take any bonsai tiles.'),
                'bon_slot-1-info': _('If you draw the card in this position, you may take 1 wood *or* 1 leaf tile.'),
                'bon_slot-2-info': _('If you draw the card in this position, you may take 1 wood *and* 1 flower tile.'),
                'bon_slot-3-info': _('If you draw the card in this position, you may take 1 leaf *and* 1 fruit tile.'),
                'bon_seishi-ref-0': _('*Wood* tiles are worth *0* points'),
                'bon_seishi-ref-1': _('*Leafs* tiles /in your bonsai/ are worth *3* points'),
                'bon_seishi-ref-2': _('*Flower* tiles /in your bonsai/ are worth *0 - 5* points || (*1* point for each side not touching any other Bonsai tile)'),
                'bon_seishi-ref-3': _('*Fruit* tiles /in your bonsai/ are worth *7* points'),
                'bon_tile-wood': _('*Wood*: must be placed adjacent to another wood tile.'),
                'bon_tile-leaf': _('*Leaf*: must be placed adjacent to a wood tile.'),
                'bon_tile-flower': _('*Flower*: must be placed adjacent to a leaf tile.'),
                'bon_tile-fruit': _('*Fruit*: must be placed in the space adjacent to two adjacent leaf tiles. *You may not place a fruit adjacent to another fruit.*'),
        ...
        updateGoalTooltips() {
            ...
            for (const { goalId, status, short } of bonsai.getGoalStatuses()) {
                ...
                const html = formatBlock('bonsai_Templates.goalTooltip', {
                    ...
                    POINTS: stringFromTemplate(applyMarkup(_('Value: *${n}* Points')), {
                        n: points,
                    }), 
                });
